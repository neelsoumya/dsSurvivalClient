...
init.studies.dataset.survival_nomissing <- function(variables)
{
  ...
  if (ds.test_env$secure_login_details)
  {
    ...
    {
      builder <- DSI::newDSLoginBuilder(.silent = TRUE)
      builder$append(server = "survival1", url = ds.test_env$ip_address_1, user = ds.test_env$user_1, password = ds.test_env$password_1, table = "SURVIVAL.EXPAND_NO_MISSING1", options=ds.test_env$options_1)
      builder$append(server = "survival2", url = ds.test_env$ip_address_2, user = ds.test_env$user_2, password = ds.test_env$password_2, table = "SURVIVAL.EXPAND_NO_MISSING2", options=ds.test_env$options_2)
      builder$append(server = "survival3", url = ds.test_env$ip_address_3, user = ds.test_env$user_3, password = ds.test_env$password_3, table = "SURVIVAL.EXPAND_NO_MISSING3", options=ds.test_env$options_3)
      ds.test_env$login.data <- builder$build()
    }
    else 
    {
